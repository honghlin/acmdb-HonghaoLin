...
//		 some code goes here
		if(pid.pgcateg() == BTreePageId.INTERNAL) {
            ...
            if (f == null) return findLeafPage(tid, dirtypages, it.next().getLeftChild(), perm, f);
            BTreeEntry entry = null;
            while(it.hasNext()) {
            	entry = it.next();
                if (f.compare(Op.LESS_THAN_OR_EQ, entry.getKey())) return findLeafPage(tid, dirtypages, entry.getLeftChild(), perm, f);
            }
            return findLeafPage(tid, dirtypages, entry.getRightChild(), perm, f); 
        }
        else if(pid.pgcateg() == BTreePageId.LEAF) return (BTreeLeafPage) getPage(tid, dirtypages, pid, perm);
        return null;
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
        ...
        int i = 0;
        for (; i < n / 2; ++i) it.next();
        Tuple midTuple = page.getTuple(n / 2);
        for (; i < n; ++i) {
            Tuple tuple = it.next();
        ...
        if (field.compare(Op.LESS_THAN_OR_EQ, midTuple.getField(keyField))) return page;
        else return newPage;
...
//        newPage.insertEntry(midEntry);
        updateParentPointers(tid, dirtypages, page);   
        updateParentPointers(tid, dirtypages, newPage);   
       
        BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), midEntry.getKey());
        BTreeEntry entry  = new BTreeEntry(midEntry.getKey(), page.getId(), newPage.getId());
        parent.insertEntry(entry);
        updateParentPointers(tid, dirtypages, parent);

        if (field.compare(Op.LESS_THAN_OR_EQ, midEntry.getKey())) return page;
        else return newPage;
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		// the corresponding parent entry.
		 int n_cur = page.getNumTuples();
	        int n_sib = sibling.getNumTuples();
	        int move = (n_cur + n_sib) / 2 - n_cur;                                       
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
        ...
        int n_cur = page.getNumEntries();
        int n_sib = leftSibling.getNumEntries();
        int move = (n_sib - n_cur) / 2;

        Iterator<BTreeEntry> it = leftSibling.reverseIterator();
        BTreeEntry p_entry = new BTreeEntry(parentEntry.getKey(),leftSibling.reverseIterator().next().getRightChild(), page.iterator().next().getLeftChild());
        p_entry.setRecordId(parentEntry.getRecordId());
        page.insertEntry(p_entry);
        
        for (int i = 0; i < move; ++i) {
            BTreeEntry entry = it.next();
            leftSibling.deleteKeyAndRightChild(entry);
            if(i < move - 1) page.insertEntry(entry);
            else parentEntry.setKey(entry.getKey());
        }
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
        ...
        int n_cur = page.getNumEntries();
        int n_sib = rightSibling.getNumEntries();
        int move = (n_sib - n_cur) / 2;

        Iterator<BTreeEntry> it = rightSibling.iterator();
        BTreeEntry p_entry = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), rightSibling.iterator().next().getLeftChild());
        p_entry.setRecordId(parentEntry.getRecordId());
        page.insertEntry(p_entry);
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
        ...
        int n_l = leftPage.getNumTuples();
        int n_r = rightPage.getNumTuples();
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
        ...
        int n_l = leftPage.getNumEntries();
        int n_r = rightPage.getNumEntries();
                       
        BTreeEntry p_entry = new BTreeEntry(parentEntry.getKey(), leftPage.reverseIterator().next().getRightChild(), rightPage.iterator().next().getLeftChild());               
        p_entry.setRecordId(parentEntry.getRecordId());
        leftPage.insertEntry(p_entry);
