...
public class JoinOptimizer {
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        int n_Joins = joins.size();
        Vector<LogicalJoinNode> plan = null;
        
        for (int i = 1; i <= n_Joins; ++i) {
            Set<Set<LogicalJoinNode>> sub = enumerateSubsets(joins, i);
                
            for (Set<LogicalJoinNode> s : sub) {
                int card = 0;
                double Min = Double.MAX_VALUE;
                for (LogicalJoinNode node : s) {
                    CostCard costcard = computeCostAndCardOfSubplan(stats, filterSelectivities, node, s, Min, pc);
                    if (costcard != null && costcard.cost < Min) {
                        Min = costcard.cost;
                        plan = costcard.plan;
                        card = costcard.card;
                    }
                }
                if (plan != null) pc.addPlan(s, Min, card, plan);
            }
        }
        
        if (explain) printJoins(plan, pc, stats, filterSelectivities);
        return plan;
